...
export function SessionStudio({ session }: { session: PracticeSession }) {
  ...
  const wideBlock = isAppsBlock

  return (
    <div className={`studio-frame${isAppsBlock ? ' apps-session-frame' : ''}${paused ? ' is-paused' : ''}`}>
      <div className={wideBlock ? 'apps-session-chrome' : undefined}>
        <header className="studio-top">
          <div>
            {wideBlock ? (
              <>
      ...
      <div className={`studio-body${wideBlock ? ' apps-session-body' : ''}`}>
        <article className={`prompt${wideBlock ? ' apps-session-prompt' : ''}`}>
          {!wideBlock ? (
            <Timer
              key={`${phase.id}-${session.phaseStartedAt ?? ''}-${session.phasePausedElapsedSec ?? 'run'}`}
              elapsedSec={timerElapsed}
