...
import { Applications } from './pages/Tracks'
...
function AppRoutes() {
  ...
  const [entered, setEntered] = useState(() => hasEnteredWorkspace())
  ...
  useEffect(() => {
    return subscribeAppAuth(() => {
      if (!isAppSignedIn() && !hasEnteredWorkspace()) {
        setEntered(false)
  ...
  return (
    <Routes>
      <Route
        path="/login"
        element={entered ? <Navigate to="/" replace /> : <WelcomeGate onEntered={onEntered} />}
      />
      <Route element={entered ? <Layout /> : <Navigate to="/login" replace />}>
